/* --------------------- PresentedCoursePage.jsx --------------------- */
import {
    Box, Grid, TextField, MenuItem, Button, Typography,
    Snackbar, Alert, Stack, Paper, CircularProgress, Divider,
  ...
  const emptyForm = {
    prof_national_id: "", course_name: "", sem_title: "",
    capacity: "", max_capacity: "", on_days: "", on_times: "", room_label: ""
  };
  ...
  export default function PresentedCoursePage() {
    ...
    const profOptions = useMemo(() => {
      ...
      return profRows.flatMap(p => {
        const m = members.find(x => x.fname === p.fname && x.lname === p.lname);
        return m
          ? [{ id: m.national_id,
    ...
    /* ───── table filter selections (semester + department) ────────── */
    const [filterSem , setFilterSem ] = useState("");
    const [filterDept, setFilterDept] = useState("");
  
    /* when lists load, pre-select the first item (for convenience) */
    useEffect(() => { if (!filterSem  && semesters.length  ) setFilterSem (semesters[0].sid); }, [semesters]);
    useEffect(() => { if (!filterDept && departments.length) setFilterDept(departments[0].did); }, [departments]);
    ...
    useEffect(() => {
      if (!filterSem || !filterDept) return;          // wait for both picks
      (async () => {
        setPcLoad(true); setPcError(null);
        try {
          const { data } = await api.get(
            `presented-courses?semester_id=${filterSem}&department_id=${filterDept}`
          );
          setPcRows(data);
        } catch (err) {
          console.error(err);
          setPcError(err.response?.data?.detail ?? "Failed to load list");
        } finally {
          setPcLoad(false);
        }
      })();
    }, [filterSem, filterDept, refreshKey]);
    ...
    const submit = async e => {
      ...
      if (!form.course_name || !form.prof_national_id || !form.sem_title) {
        setSnack({open:true,msg:"Course, Professor, Semester required",sev:"error"});
        return;
      }
      if (+form.capacity > +form.max_capacity) {
        setSnack({open:true,msg:"Capacity > Max Capacity",sev:"error"}); return;
      }
      ...
      try {
        ...
        setSnack({open:true,msg:"Presented course created ✔︎",sev:"success"});
        setForm(emptyForm);
    ...
    const busy =
      !courses.length || !semesters.length || !rooms.length || !profOptions.length;
      ...
      <Box sx={{ p:{ xs:2, md:4 } }}>
        <AdminCard>
            ...
            {busy ? (
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={6}>
                  <TextField select fullWidth required label="Course" sx={{ mb:2 }}
                  ...
                  <TextField select fullWidth required label="Professor" sx={{ mb:2 }}
                  ...
                  <TextField select fullWidth required label="Semester" sx={{ mb:2 }}
                    value={form.sem_title} onChange={handle("sem_title")}>
                    {semesters.map(s=>(
                ...
                <Grid item xs={12} md={6}>
                  ...
                  <TextField select fullWidth required label="Room" sx={{ mb:2 }}
          ...
          <Stack direction={{ xs:"column", sm:"row" }} spacing={2} sx={{ mb:2 }}>
            <TextField select label="Semester" size="small" sx={{ minWidth:180 }}
              value={filterSem} onChange={e=>setFilterSem(e.target.value)}>
              {semesters.map(s=>(
                <MenuItem key={s.sid} value={s.sid}>{s.sem_title}</MenuItem>
              ))}
            </TextField>
            <TextField select label="Department" size="small" sx={{ minWidth:220 }}
              value={filterDept} onChange={e=>setFilterDept(e.target.value)}>
              {departments.map(d=>(
                <MenuItem key={d.did} value={d.did}>{d.department_name}</MenuItem>
              ))}
            </TextField>
          </Stack>
          ...
          {pcLoad ? (
            ...
            <Typography color="error" variant="body2">{pcError}</Typography>
          ) : pcRows.length === 0 ? (
            <Typography color="text.secondary">No records for this filter.</Typography>
          ) : (
            <Box sx={{ maxHeight:320, overflow:"auto", borderRadius:1, boxShadow:1 }}>
              <Table size="small"
                     sx={{ "& tbody tr:nth-of-type(odd)":{ bgcolor:"action.hover" }}}>
                <TableHead>
                  <TableRow>
                ...
                <TableBody>
                  {pcRows.map(r=>(
                    <TableRow key={r.pcid}>
                      ...
                      <TableCell align="right">{r.capacity} / {r.max_capacity}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
